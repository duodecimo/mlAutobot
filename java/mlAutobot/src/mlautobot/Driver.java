 ...
 * receiving streamed image frames from a video streaming app on a phone and a
 * user interface allowing a human driver to control the car with the cursor
 * keys or mouse, sent to an RC unit over the serial interface.
 *
 * RECORD MODE
 *
 * In record mode, the video frames are saved to disk, labelled with the current
 * control input coming from the human driver. The neural network is trained
 * using these labelled frames in a separate environment on the computer.
 * Trained parameters are saved out to files which are in turn read by the
 * Driver app...
 *
 * AUTO MODE
 *
...
public class Driver extends JFrame implements FeatureCallback, KeyListener,
    ...
    public Driver(/*SerialWrapper serial,*/File featureOutFile, String theta1File,
        ...
        //featureWriter = new FeatureWriter(featuresOut);
        //writeThread = new Thread(featureWriter);
        //writeThread.start();
    }
